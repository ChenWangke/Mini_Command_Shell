...
(4) Run      valgrind --track-origins=yes --leak-check=full --show-leak-kinds=all ./myShell
...
Among the environment list we can find  PATH=/bin:/usr/bin

which is correct because we set PATH to /bin:/usr/bin, we can find the command env in this directory.

So we can use env again. and "/bin:/usr/bin"  means PATH is changed by us.
